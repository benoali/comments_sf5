...
class FacebookAuthenticator extends SocialAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        if(!$email){
            $email = "{$facebookData->getId()}@githuboauth.com";
        }
        ...
        if ($existingUser) {
            ...
            if (!$user) {
                ...
                $user->setName($facebookData["name"]);
                $user->setPassword("123");
                $user->setFacebookId($facebookData['id']);
                $user->setAvatar($facebookData['picture_url']);
                $user->setEmail($email);
                //$user->setHostedDomain('lllllll');
            }
        }
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        $targetUrl = $this->router->generate('app_article');
